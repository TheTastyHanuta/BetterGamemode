		...
		if(sender instanceof Player) {
			...
			if(cmd.getName().equalsIgnoreCase("gamemode")){
				if(p.hasPermission("system.gm")){
					if(args.length == 0){
						p.sendMessage(Mainclass.prefix + "§cVerwende /gamemode 0/1/2/3");
						...
						if(p2 == null) {
							p.sendMessage(Mainclass.prefix+ "§cDer angegebene Spieler ist nicht online! Verwende /gamemode 0/1/2/3 <Spieler>!");
						}else {
						...
						if(args[0].equalsIgnoreCase("0")){
							...
							p.sendMessage(Mainclass.prefix + "§cVerwende /gamemode 0/1/2/3 <Spieler>!");
						}
						}
					}else {
						p.sendMessage(Mainclass.prefix + "§cVerwende /gamemode 0/1/2/3 <Spieler>!");
				...
				if(args.length ==2) {
					Player p2 = Bukkit.getPlayer(args[1]);
					if(p2 == null) {
						Bukkit.getConsoleSender().sendMessage(Mainclass.prefix+ "§cDer angegebene Spieler ist nicht online! Verwende /gamemode 0/1/2/3 <Spieler>!");
					}else {
				...
				if(args[0].equalsIgnoreCase("0")){
					...
					Bukkit.getConsoleSender().sendMessage(Mainclass.prefix + "§cVerwende /gamemode 0/1/2/3 <Spieler>!");
					}
					}
				}else {
					Bukkit.getConsoleSender().sendMessage(Mainclass.prefix + "§cVerwende /gamemode 0/1/2/3 <Spieler>!");
